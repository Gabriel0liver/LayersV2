...
function TargetsTab(parameter, parameter_key)
    ...
    SliderParameter(parameter,parameter_key)
    ---- Button add track
    if reaper.ImGui_Button(ctx, 'Add Track Volume', -FLTMIN) then
        ...
        AddSelectedTracksToTargetsVolume(FocusedProj,parameter.targets)    
       
        is_save = true
    end

    
    SelectFxModalGab(parameter)
    --Button Add Effect Param
    if reaper.ImGui_Button(ctx, 'Add Fx Param', -FLTMIN) then
        -- if holding alt then set targets to selected tracks, else add to the current targets
        if reaper.ImGui_GetKeyMods(ctx) == reaper.ImGui_Mod_Alt() then 
            parameter.targets = {}
        end
        if reaper.CountSelectedTracks(FocusedProj) ~= 1 then
            print("select only one track")
            return
        else
            parameter.selectedTrack = reaper.GetSelectedTrack(FocusedProj, 0)
            reaper.ImGui_OpenPopup(ctx, 'SelectFx')
            is_save = true
        end
    end
...
--[[
    ...
    for targetName, target in pairs(parameter.targets) do
        ...
        if(target.volume) then
            open = reaper.ImGui_TreeNode(ctx, 'Track : '..name..'\nVolume')
        else
            local _, paramName = reaper.TrackFX_GetParamName(track, reaper.TrackFX_GetByName(track, target.fx, false), target.param)
            open = reaper.ImGui_TreeNode(ctx, 'Track : '..name..'\nFX : '.. target.fx .. '\nParameter : ' .. paramName)
        end
        ...
        -- Right click node
        TargetRightClick(parameter,target,track,targetName)
...
function TargetRightClick(parameter,target,track,targetName)
    ...
    if reaper.ImGui_BeginPopupContextItem(ctx) then
        ...
        if reaper.ImGui_Button(ctx, 'Remove Target',-FLTMIN) then
            RemoveTarget(parameter, track, targetName)
            SaveProjectSettings(FocusedProj, ProjConfigs[FocusedProj]) 
...
function SelectFxModalGab(parameter)
    local track = parameter.selectedTrack
    local fx_id = parameter.selectedFx
    if reaper.ImGui_BeginPopupModal(ctx,'SelectFx',true) then
        local fx_count = reaper.TrackFX_GetCount(track)
        for fx_idx = 0, fx_count-1 do
            local _, fx_name = reaper.TrackFX_GetFXName(track, fx_idx)
            if reaper.ImGui_BeginMenu(ctx, fx_name) then
                local a,b = SelectParamMenuGab(parameter,track,fx_idx,fx_name)
                reaper.ImGui_EndMenu(ctx)
            end
        end
        reaper.ImGui_EndPopup(ctx)
...
function SelectParamMenuGab(parameter,track,fx_id,fx_name)
    reaper.ImGui_MenuItem(ctx, 'Select Parameter', nil, false, false)
    local fx_count = reaper.TrackFX_GetNumParams(track, fx_id)
    for param_idx = 0, fx_count-1 do
        local _, param_name = reaper.TrackFX_GetParamName(track, fx_id, param_idx, '')
        if reaper.ImGui_MenuItem(ctx, param_name) then
            AddFxParamToTarget(FocusedProj, parameter.targets, track, fx_name, param_idx)
        end
    end
end
