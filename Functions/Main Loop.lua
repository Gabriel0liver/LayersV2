            ...
            for track, target in pairs(parameter.targets) do
                if (target.value ~= parameter.true_value) or target.is_update_ce or IsFirstRun then -- if the value from the target is different from the parameter, if the curve was updated, if the script was initialized in this frame.
                    ...
                    if(target.volume) then
                        UpdateLayerFXVolumeValue(target, target.track)
                    else
                        UpdateLayerFXParamValue(target, target.track)
                    end
                end
            end
        end
...
function CheckProjects()
    ...
    for proj, proj_table in pairs(ProjConfigs) do
        ...
        for parameter_idx, parameter in ipairs(proj_table.parameters) do
            for track_name, target in pairs(parameter.targets) do
                if not reaper.ValidatePtr2(proj, target.track, 'MediaTrack*') then -- string means it couldnt be loaded when opening the script, from the save. Remove this target
                    parameter.targets[track_name] = nil
                    goto continue2
                end
                if not target.volume and reaper.TrackFX_GetByName(target.track, target.fx, false) == -1 then -- FX not found
                    parameter.targets[track_name] = nil
                    goto continue2
                end
                -- Check FX
                CheckFxPos(target.track, target, proj)
                ::continue2::
            end
