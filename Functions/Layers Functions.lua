...
function CheckFxPos(track, target, proj)
    local fx_idx = CheckLayerFX(target.track, true) -- If there is not an FX then add it at the end
    ...
    if target.is_force_fx_settings then
        local fx_idx = CheckLayerFX(target.track, false)
        -- MIDI Chase
        local param = 1
...
function UpdateLayerFXVolumeValue(target, track)
    local fx_idx = CheckLayerFX(target.track, false)
    local fx_value = ce_evaluate_curve(target.curve,target.value)
    reaper.TrackFX_SetParamNormalized(track, fx_idx, 0, fx_value)
end

function UpdateLayerFXParamValue(target, track)
    local fx_idx = reaper.TrackFX_GetByName(track, target.fx, false)
    local param_id = target.param
    local fx_value = ce_evaluate_curve(target.curve,target.value)
    reaper.TrackFX_SetParamNormalized(track, fx_idx, param_id, fx_value)
end
...
function AddSelectedTracksToTargetsVolume(proj, targets)
    for track in enumSelectedTracks(proj) do
        local targetName = tostring(track)..'-Volume'
        local target_table = CheckTargetsForTrack(proj,targetName)
        targets[targetName] = target_table or CreateTargetTableVolume(track)
        -- FX
        CheckLayerFX(track, true)
    end
end

function AddFxParamToTarget(proj, targets, track, fx, param)
    local targetName = tostring(track)..tostring(fx)..tostring(param)
    local target_table = CheckTargetsForTrack(proj,targetName)
    targets[targetName] =  target_table or CreateTargetTableFx(track,fx,param)
    -- FX
    --CheckLayerFX(track, true)
end
...
function CheckTargetsForTrack(proj,targetName)
    for parameter_index, parameter in ipairs(ProjConfigs[proj].parameters) do
        for target_name, target in pairs(parameter.targets) do
            if target_name == targetName then
                parameter.targets[targetName] = nil
                return target
            end
        end
    end
...
function RemoveTarget(parameter, track, targetName)
...
function CreateTargetTableVolume(track)
    local t = {
...
function CreateTargetTableFx(track,fx,param)
    local t = {
        curve = CreatePointTable(), ---- TODO start with a linear value
        bypass = false,
        track = track,
